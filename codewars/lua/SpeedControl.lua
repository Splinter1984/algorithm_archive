...
per hour obtained on the sections of x. If x length is less than or equal to 1 return 0 since the car didn't move.
...
function solution.gps(s, xs)
  local speed = {}
  if not next(xs) or #xs < 2 then return 0 end
  for i = 1, #xs - 1 do
    speed[i] = math.abs(xs[i+1] - xs[i]) * 3600/s
  end
  table.sort(speed)
  return math.floor(speed[#speed])
end
