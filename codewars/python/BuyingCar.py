...
def nbMonths(startPriceOld, startPriceNew, savingperMonth, percentLossByMonth):
    ...
    month_count = 0
    tmp = 0
    while (startPriceOld + bank - startPriceNew < 0):
        bank+= savingperMonth
        month_count+=1
        if month_count % 2 == 0:
            tmp+= 0.005
        startPriceNew = startPriceNew * (1.0 - tmp - percentLossByMonth * 0.01)
        startPriceOld = startPriceOld * (1.0 - tmp - percentLossByMonth * 0.01)    

    return [month_count, round(startPriceOld + bank - startPriceNew)]
